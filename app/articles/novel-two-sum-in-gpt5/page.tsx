...
export default function NovelTwoSumInGPT5() {
  const articlePath = path.join(path.join(process.cwd(), "app/articles/novel-two-sum-in-gpt5/"));
  const finalSpecification = useMemo(() => {
    return fs.readFileSync(path.join(articlePath, "finalSpecification.md"), "utf8");
  }, [articlePath]);
  const fullInterpreter = useMemo(() => {
    return fs.readFileSync(path.join(articlePath, "fullInterpreter.md"), "utf8");
  }, [articlePath])
  return (
