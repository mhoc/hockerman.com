...
export async function SpotifyTrendingSongsCard() {
  ...
  return (
    <div className="flex flex-col">
      <span className="text-md text-cobalt-100 font-semibold">Trending</span>
      {results.length === 0 && <span className="text-sm text-cobalt-400">No trending songs</span>}
      {results.slice(0, 4).map((result: any) => (
        <div className="flex flex-row gap-2 items-center" key={`${result.track_name}-${result.artist_1_name}`}>
          ...
          <span className="text-sm text-cobalt-300">{result.track_name} </span>
          <span className="text-sm text-cobalt-500">&nbsp;{result.artist_1_name}</span>
        </div>
      ))}
    </div>
  );
